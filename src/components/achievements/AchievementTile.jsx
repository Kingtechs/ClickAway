...
export default function AchievementTile({ achievement }) {
  if (!achievement) {
    return <div className="achievementTilePlaceholder" aria-hidden="true" />
  }
  ...
  return (
    <article
      className={`achievementTile ${isUnlocked ? "isUnlocked" : "isLocked"}`}
      aria-label={`${title}. ${description}. ${percentText}.`}
    >
      <header className="achievementTileHeader">
        <span className="achievementIconWrap">
          <AchievementIcon iconKey={iconKey} />
        </span>
        <span className={`achievementState ${isUnlocked ? "isUnlocked" : "isLocked"}`}>
